...
class EditProfileForm(FlaskForm):
    ...
    gender = RadioField('Gender: ', choices=[('0', 'Male'), ('1', 'Female'), ('2', 'Unknown')], validators=[DataRequired()])
    ...
    def validate_username(self, field):
        user_found = User.query.filterby(username=field.data).first()
        if user_found is not None and user_found.id != self.user.id:
            raise ValidationError('New username already used.')

    def validate_email(self, field):
        user_found = User.query.filterby(email=field.data).first()
        if user_found is not None and user_found.id != self.user.id:
